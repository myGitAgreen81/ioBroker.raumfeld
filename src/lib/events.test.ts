...
describe('parseDidlLite', () => {
	it('liest die Titelangaben aus dem eingebetteten DIDL-Lite', () => {
		...
		// Beide Felder tragen Attribute und kommen deshalb als Objekt aus dem
		// Parser - ohne besondere Behandlung stuende hier "[object Object]".
		expect(track?.artist).to.equal('Testkuenstler');
		expect(track?.albumArtUri).to.equal('https://music.cdn.raumfeld.com/rock.jpg');
